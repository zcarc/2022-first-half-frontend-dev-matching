...
첫번째 매개변수 start는 0을 시작으로 배 열에서 요소를 추출할 시작 인덱스를 의미한다.

- 음수 인덱스가 전달되면 배열의 끝에서부터의 길이는 의미한다. -2가 전달되면 배열의 끝에서 두번째 인덱스부터 시작한다는 것을 의미한다.
- start가 undefined(매개변수를 생략하거나, undefined, null, empty를 전달)인 경우 0부터 끝까지 추출한다. <br />
  (두번째 매개변수를 전달하지 않을 때 유효하다. 만약 두번째 매개변수 end를 전달한다면 start 매개변수의 값도 같이 전달되어야 한다.)
...
- 음수 인덱스가 전달되면 배열의 끝에서부터의 길이를 의미한다. 음수가 아닌 양수일 때도 추출을 해당 종료 인덱스까지 포함하지 않은 것 처럼 음수일 경우에도 해당 종료 인덱스를 포함하지 않고 바로 앞 인덱스까지만 추출한다.
...
```js
// start, end 매개변수 모두를 전달해서 3개의 요소를 추출
const arr = [10, 20, 30, 40, 50];
let res = arr.slice(2, 5);

console.log(arr); // [10, 20, 30, 40, 50] (변경되지 않음)
console.log(res); // [30, 40, 50]
```

```js
// end 매개변수를 생략해서 배열의 start 인덱스 요소부터 배열의 끝 요소까지 추출
res = arr.slice(2);
console.log(res); // [30, 40, 50]
```

```js
// start 매개변수에 음수를 전달해서 배열의 끝에서부터 인덱스를 시작해서 요소를 추출
res = arr.slice(-2);
console.log(res); // [40, 50]
```

```js
// end 매개변수에 음수를 전달해서 배열의 끝에서부터 인덱스를 시작해서 양수인 경우와 동일하게 바로 앞까지 요소를 추출
res = arr.slice(2, -1);
console.log(res); // [30, 40]
```

```js
// start가 undefined(매개변수를 생략하거나, undefined, null, empty를 전달)인 경우 0부터 끝까지 추출
res = arr.slice();
...
```js
// start가 배열의 인덱스 범위(0 ~ 4)보다 크다면 빈 배열 반환
res = arr.slice(5);
console.log(res); // []
...
```js
// end가 배열의 길이(5)보다 크다면 요소를 배열의 끝까지 추출
res = arr.slice(1, 15);
