...
첫번째 매개변수 start는 0을 시작으로 배 열에서 요소를 추출할 시작 인덱스를 의미한다.
  ...
  - start가 undifiend(매개변수를 전달하지 않은 경우)인 경우 0부터 끝까지 추출한다. <br />
    (start 매개변수 하나만 전달하지 않을 때 유효하다. 만약 두번째 매개변수 end를 전달한다면 start 매개변수는 무조건 인덱스 값을 전달해야한다.) 
...
```js
// start가 undifiend(매개변수를 전달하지 않은 경우)인 경우 0부터 끝까지 추출
res = arr.slice();
